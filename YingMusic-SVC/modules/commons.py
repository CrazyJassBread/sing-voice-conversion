...
def build_model(args, stage="DiT"):
    if stage == "DiT":
        ...
        length_regulator = InterpolateRegulator(
            ...
            use_style_residual = args.length_regulator.use_style_residual if hasattr(args.length_regulator, "use_style_residual") else False,
            residual_alpha = args.length_regulator.residual_alpha if hasattr(args.length_regulator, "residual_alpha") else 0,
        )
        cfm = CFM(args)
        nets = Munch(
            cfm=cfm,
...
def my_load_checkpoint(
    ...
    *,
    ...
    assert ckpt_path and (ckpt_path.endswith(".pth") or ckpt_path.endswith(".pt")) and \
           os.path.exists(ckpt_path), f"Checkpoint 不存在: {ckpt_path}"

    state = torch.load(ckpt_path, map_location="cpu")
    # 1. 加载网络参数
    net_state = state.get("net", {})
    for key in model:
        if key in ignore_modules or key not in net_state:
            continue
        sd = net_state[key]
        ...
        if missing:
            print(f"[my_load_checkpoint] 模块 {key} 有参数 {missing} 因尺寸不符而跳过")
        model[key].load_state_dict(loadable, strict=False)
        print(f"[my_load_checkpoint] 模块 {key} 权重加载完毕")
    # 网络切 eval，避免 BN/Dropout 干扰
    _ = [m.eval() for m in model.values()]
    # 2. 如需继续训练，恢复优化器 & scheduler
    if not load_only_params:
        ...
        if "optimizer" in state:
            try:
                ...
                print("[my_load_checkpoint] 优化器状态已恢复")
            except Exception as e:
                print(f"[my_load_checkpoint] 优化器状态恢复失败: {e}")
        if "scheduler" in state:
            try:
                scheduler.load_state_dict(state["scheduler"])
                print("[my_load_checkpoint] Scheduler 状态已恢复")
            except Exception as e:
                print(f"[my_load_checkpoint] Scheduler 状态恢复失败: {e}")
    else:
        epoch, iters = 0, 0
    return model, optimizer, scheduler, epoch, iters
